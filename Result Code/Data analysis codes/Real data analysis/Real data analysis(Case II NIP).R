#The results of the real data analysis in Case II under NIP.
...
llfbayesmiu <- (-1/p)*log(mean(exp(-p*gibbsmiu[100:1000])))
llfbayeslamda <- (-1/p)*log(mean(exp(-p*gibbslamda[100:1000])))
gelfbayesmiu <-(mean((gibbsmiu[100:1000])^(-q)))^(-1/q)
gelfbayeslamda <-(mean((gibbslamda[100:1000])^(-q)))^(-1/q)
...
path <- file.path("E://progress result1119","case2realdataEstimateNIP.xls")
