#The results of the real data analysis in Case I under NIP.
...
miuhat1<-(NRvalue$root)[2]
lamdahat1 <- (NRvalue$root)[1]
...
bigmiuCI<-deltaCI(miuhat1,varmiu,alpha = 0.05)
biglamdaCI<-deltaCI(lamdahat1,varlamda)
#EM
...
ff <- data.frame(MLElamda=NRvalue$root[1],MLEmiu=NRvalue$root[2],
                 ...
                 row.names = '估计值')
path <- file.path("E://progress result1119","case1realdataEstimateNIP.xls")
