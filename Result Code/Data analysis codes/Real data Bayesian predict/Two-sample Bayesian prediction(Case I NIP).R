#Bayesian two-sample prediction for the real data in Case I under NIP. 
...
function4 <- function(t,a1,s){
  ...
  for(h in 1:Nsim11){
    premiu[h]<- ars(1,logPaimiu,dlogPaimiu,x=0.1,m=1,lb=TRUE,xlb=0.01,ub=TRUE,xub=X[1])
    Gmiu<- sum((R1[1:JX]+1)*(X-premiu[h])^2)+RXJ*(TimX-premiu[h])^2
    prelamda[h] <- rgamma(1,shape = (JX+a),rate = (b+Gmiu))
    for (q2 in 0:(s-1)) {
      if(q2==(s-1)){
        Mqs <- N2
...
library(aqp)
sink("E:\\progress result1119\\twosampleprecase1NIP.txt")
ff
sink()
