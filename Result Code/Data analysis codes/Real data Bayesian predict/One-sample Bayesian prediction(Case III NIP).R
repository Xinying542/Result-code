#Bayesian one-sample prediction for the real data in Case III under NIP. 
...
c=10
Tim=1.3  
Datah<-hybriddata(Tim=1.3,m=14,c=10,datareal,R1)
a <-0
b <-0
X=Datah$X
JX =Datah$JX
...
onesampleprevalue[c(4,8,14)]

preMatrixL <- matrix(ncol=4,nrow=14)
a1=0.975
for (j in c(4,8,14)) {
  if(j==8){
    for (i in 1:lastR[j]) {
...
ff <- list(point=onesampleprevalue[c(4,8,14)],ETL=preMatrixL[c(4,8,14),],ETU=preMatrixU[c(4,8,14),])
library(aqp)
sink("E:\\progress result1119\\onesampleprecase3NIP.txt")
ff
sink()
